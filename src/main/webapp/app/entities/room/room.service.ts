...
export class RoomService {
  ...
  protected convertDateFromClient(room: IRoom): IRoom {
    const copy: IRoom = Object.assign({}, room, {
      ...
      updatedDate: room.updatedDate && room.updatedDate.isValid() ? room.updatedDate.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((room: IRoom) => {
        ...
        room.updatedDate = room.updatedDate ? moment(room.updatedDate) : undefined;
      });
    }
    return res;
  }
